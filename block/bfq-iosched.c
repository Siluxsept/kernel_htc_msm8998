		      ...
		      struct bfq_io_cq *bic, bool bfq_already_existing)
{
	...
	if (bfqq->wr_coeff > 1 && bfqq->weight_raise < 2 &&
	...
	if (old_wr_coeff == 1 && bfqq->wr_coeff > 1) {
		if (bfqd->wr_busy_queues < bfqd->busy_queues)
			bfqd->wr_busy_queues++;
	} else if (old_wr_coeff > 1 && bfqq->wr_coeff == 1) {
		if (bfqd->wr_busy_queues > 0)
			bfqd->wr_busy_queues--;
	}
}
					     ...
					     bool soft_rt)
{
	if (old_wr_coeff == 1 && wr_or_deserves_wr) {
		...
		if (interactive) {
			bfqq->wr_coeff = bfqd->bfq_wr_coeff * bfqq->weight_raise;
		...
		bfqq->entity.budget = min_t(unsigned long,
					    bfqq->entity.budget,
					    2 * bfq_min_budget(bfqd)) * bfqq->weight_raise;
		...
		if (interactive) { /* update wr coeff and duration */
			bfqq->wr_coeff = bfqd->bfq_wr_coeff * bfqq->weight_raise;
			bfqq->wr_cur_max_time = bfq_wr_duration(bfqd);
		} else if (in_burst && bfqq->weight_raise < 2) {
			bfqq->wr_coeff = 1;
			bfq_log_bfqq(bfqd, bfqq,
					     ...
					     bool *interactive)
{
	...
	soft_rt = bfqd->bfq_wr_max_softrt_rate > 0 && bfqq->weight_raise &&
		!in_burst &&
		time_is_before_jiffies(bfqq->soft_rt_next_start);
	*interactive =
		!in_burst && bfqq->weight_raise &&
		idle_for_long_time;
...
static void bfq_bfqq_end_wr(struct bfq_queue *bfqq)
{
	...
	if (bfq_bfqq_busy(bfqq) && bfqq->wr_coeff > 1) {
		if (bfqq->bfqd->wr_busy_queues > 0)
				     ...
				     enum bfqq_expiration reason)
{
	...
	if (bfqq->wr_coeff == 1)
		budget = bfqq->max_budget;
	      ...
	      */
		budget = 2 * min_budget * bfqq->weight_raise;
...
static bool bfq_bfqq_may_idle(struct bfq_queue *bfqq)
{
	...
	return bfq_bfqq_sync(bfqq) && bfqq->weight_raise == 2 &&
		(idling_boosts_thr_without_issues ||
		 idling_needed_for_service_guarantees);
}
...
static void bfq_update_wr_data(struct bfq_data *bfqd, struct bfq_queue *bfqq)
{
	...
	if (bfqq->wr_coeff > 1) { /* queue is being weight-raised */
		 ...
		 */
		if (bfq_bfqq_in_large_burst(bfqq) && bfqq->weight_raise < 2)
			bfq_bfqq_end_wr(bfqq);
		else if (time_is_before_jiffies(bfqq->last_wr_start_finish +
					   bfqq->wr_cur_max_time)) {
			if (bfqq->weight_raise < 2 && (bfqq->wr_cur_max_time != bfqd->bfq_wr_rt_max_time ||
				       ...
				       struct bfq_io_cq *bic)
{
	...
	if (bfqq) {
		...
		if (is_sync) {
			bfqq->weight_raise = bfqgd->weight_raise;
			if (bfqq->weight_raise){
				bfqq->wr_coeff = bfqd->bfq_wr_coeff * bfqq->weight_raise;
			}
			bfqq->entity.weight = bfqq->entity.orig_weight = bfqq->entity.new_weight = bfqgd->weight;
		}
